...
function StatCard({ label, value, icon: Icon, href }: StatCardProps) {
  const content = (
    <div className="flex items-start justify-between gap-3 rounded-xl border border-zinc-200 bg-white p-4 shadow-sm sm:p-5">
      <div>
        <p className="text-sm font-medium text-zinc-500">{label}</p>
        <p className="mt-1 text-2xl font-semibold text-zinc-900 sm:text-3xl">{value}</p>
      </div>
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl font-bold text-zinc-900 sm:text-2xl">Overview</h1>
          <p className="mt-1 text-sm text-zinc-500">Trip and booking statistics</p>
        </div>
        <Link
      ...
      <section className="rounded-xl border border-zinc-200 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-zinc-200 px-4 py-3 sm:px-5">
          <h2 className="text-sm font-semibold text-zinc-900">Recent trips</h2>
          <Link
            href="/dashboard/trips"
        ...
        <div className="divide-y divide-zinc-100">
          {recentTours.length === 0 ? (
            <div className="px-4 py-8 text-center text-sm text-zinc-500 sm:px-5">
              No trips yet.{" "}
              <Link href="/dashboard/trips/create" className="font-medium text-zinc-700 hover:underline">
            ...
            recentTours.map((tour) => (
              ...
              >
                <div className="min-w-0 flex-1">
                  <p className="truncate font-medium text-zinc-900">{tour.title}</p>
                  <p className="mt-0.5 text-xs text-zinc-500">{tour.durationLabel}</p>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-zinc-400">{tour._count.days} days</span>
                </div>
              </Link>
            ))
          )}
